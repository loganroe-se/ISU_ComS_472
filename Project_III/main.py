...
class Task:
    ...
    def __init__(self, id, running_id, max_iter=1000):
        self.id = id
        self.running_id = running_id
        self.max_iter = max_iter

        # Randomize probabilities for each task instantiation
        raw_probs = np.random.rand(3)
        self.prob = raw_probs / raw_probs.sum()
    ...
    def run(self):
        ...
        self.save_log()
        # print("Actual probability: ", self.prob)
...
if __name__ == "__main__":
    ...
    for id in range(100):
        for running_id in range(2):
            ...
            sum[2] += result[2]
        if id % 10 == 0 and id != 0:
            print(sum[0]/(id*2), sum[1]/(id*2), sum[2]/(id*2))
    print(sum[0]/200, sum[1]/200, sum[2]/200) 